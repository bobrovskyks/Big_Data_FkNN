...
import org.apache.spark.ml.HS_FkNN.HS_FkNN._
...
private[ml] abstract class Tree extends Serializable {
  ...
  def query(v: VectorWithNorm, k: Int): Iterable[(RowWithVector, Double)] = query(new HS_FkNNCandidates(v, k)).toIterable

  /**
   * Refine k-NN candidates using data in this [[Tree]]
   */
  private[HS_FkNN] def query(candidates: HS_FkNNCandidates): HS_FkNNCandidates
   ...
   */
  private[HS_FkNN] def distance(candidates: HS_FkNNCandidates): Double = distance(candidates.queryVector)

  private[HS_FkNN] def distance(v: VectorWithNorm): Double =
    if (pivot.vector.size > 0) pivot.fastDistance(v) else 0.0
}

private[HS_FkNN] case object Empty extends Tree {
  ...
  override def query(candidates: HS_FkNNCandidates): HS_FkNNCandidates = candidates
}

private[HS_FkNN] case class Leaf(data: IndexedSeq[RowWithVector],
  ...
  override def query(candidates: HS_FkNNCandidates): HS_FkNNCandidates = {
    val sorted = data
 ...
 * faster than radius).
 */
private[HS_FkNN] case class MetricTree(leftChild: Tree,
 ...
 */
private[HS_FkNN] case class SpillTree(leftChild: Tree,
                                       leftPivot: VectorWithNorm,
                                       rightChild: Tree,
                                       rightPivot: VectorWithNorm,
                                       pivot: VectorWithNorm,
 ...
 */
private[HS_FkNN] class HS_FkNNCandidates(val queryVector: VectorWithNorm, val k: Int) extends Serializable {
  private[HS_FkNN] val candidates = mutable.PriorityQueue.empty[(RowWithVector, Double)] {
    Ordering.by(_._2)
  }
